...
- **Existing users are not told about the pricing change.** `stems`, `midi` and `remaster` were free and now cost credits. Both routers documented them as free by design; the story overrides that, and someone should decide how it is communicated before this reaches production. Out of scope for the code, but not something to leave unsaid.
