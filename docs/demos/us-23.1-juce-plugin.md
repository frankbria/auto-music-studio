...
| Non-audio-thread infrastructure for HTTP; no network on the audio thread | `BackgroundTaskQueue`; `processBlock` is passthrough only. 14/14 unit tests, including proof that queued work runs on a different thread and that results marshal to the message thread. Two mutations (removing the channel clear; running `enqueue` inline) were both caught by the suite | **VERIFIED** |
