...
something the musician never saw.

The column is `credits_used`, not `credits`: it is signed as **spent**, so the grant reads
`-47.5` here while the table shows `+500` — the table answers "what happened to my
balance", the export answers "what did this cost", and the latter is the one that has to
sum. The header names the convention rather than leaving it to be inferred from a row.
Titles containing commas or quotes are quoted and escaped, and a title opening with
`= + - @` is prefixed so a spreadsheet does not run it as a formula.
