...
- **AC1's badges were verified on song detail, not on the clip card** (#404). `ClipCard`'s
  ⋯ menu badges only **Open in Editor**; `Send to Mastering`, `Export to DAW` and the
  Download submenu render without one, because that menu hardcodes its own structure
  instead of rendering from the shared registry. Behaviour is right — clicking any of them
  opens the upgrade prompt, verified above — so what is missing is the *advance* signal.
  Pre-existing rather than introduced here (`main` renders those items badge-less too), and
  left for a follow-up rather than widening this PR further. Called out explicitly because
  it is the same shape as the AC2 gap above: a criterion verified on one surface and not
  the other.
