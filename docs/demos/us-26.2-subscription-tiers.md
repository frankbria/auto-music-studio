...
| **AC2. Free user clicking a Pro feature sees an upgrade modal (not an error)** | Clicked **Send to Mastering**, then **Export to DAW**, as `free@`. | A dialog opens headed **"Mastering is a Pro feature"** / "Upgrade to master your tracks to a professional loudness target", listing the six Pro benefits, with **Not now** and **See plans** → `/settings/billing`. The DAW item produces different, correct copy — **"Studio editing is a Pro feature"** — proving the copy is keyed per capability rather than generic. The items are **not `disabled`**, so the click actually fires; no navigation, no error toast, URL unchanged. | **VERIFIED** |
