...
class TestSpendOrder:
    ...
    async def test_a_deduction_within_the_monthly_bucket_leaves_purchases_alone(self) -> None:
        user = await make_user("buckets-monthly@example.com", credits_balance=50.0, purchased_credits=100.0)
    ...
    async def test_a_deduction_spanning_both_drains_monthly_first(self) -> None:
        user = await make_user("buckets-spanning@example.com", credits_balance=10.0, purchased_credits=100.0)
    ...
    async def test_a_deduction_exactly_equal_to_the_monthly_balance(self) -> None:
        ...
        user = await make_user("buckets-exact@example.com", credits_balance=25.0, purchased_credits=100.0)
    ...
    async def test_purchased_credits_alone_can_pay(self) -> None:
        user = await make_user("buckets-onlybought@example.com", credits_balance=0.0, purchased_credits=5.0)
    ...
    async def test_the_split_variant_reports_what_came_from_purchases(self) -> None:
        ...
        user = await make_user("buckets-split@example.com", credits_balance=10.0, purchased_credits=100.0)
    ...
    async def test_the_split_variant_reports_zero_within_the_monthly_bucket(self) -> None:
        user = await make_user("buckets-split-monthly@example.com", credits_balance=50.0, purchased_credits=100.0)
    ...
    async def test_an_unaffordable_deduction_changes_nothing(self) -> None:
        user = await make_user("buckets-broke@example.com", credits_balance=1.0, purchased_credits=1.0)
...
class TestAtomicity:
    async def test_concurrent_deductions_cannot_overdraw_across_buckets(self) -> None:
        ...
        user = await make_user("buckets-race@example.com", credits_balance=3.0, purchased_credits=2.0)
    ...
    async def test_a_legacy_document_without_either_field_still_works(self) -> None:
        ...
        user = await make_user("buckets-legacy@example.com", credits_balance=10.0)
    ...
    async def test_a_document_with_no_purchased_field_deducts_from_monthly(self) -> None:
        user = await make_user("buckets-nopurchased@example.com", credits_balance=10.0)
...
class TestGrant:
    ...
    async def test_a_grant_lands_in_the_purchased_bucket(self) -> None:
        user = await make_user("buckets-grant@example.com", credits_balance=50.0)
    ...
    async def test_grants_accumulate(self) -> None:
        user = await make_user("buckets-grant2@example.com")
    ...
    async def test_a_non_positive_grant_is_refused(self, amount: float) -> None:
        ...
        user = await make_user("buckets-badgrant@example.com")
...
class TestMonthlyResetIgnoresPurchases:
    ...
    async def test_the_reset_does_not_touch_the_purchased_bucket(self) -> None:
        user = await make_user("buckets-reset@example.com", credits_balance=0.0, purchased_credits=100.0)
    ...
    async def test_the_full_allocation_arrives_despite_a_large_purchase(self) -> None:
        ...
        user = await make_user("buckets-reset2@example.com", credits_balance=0.0, purchased_credits=500.0)
        await User.get_pymongo_collection().update_one({"_id": user.id}, {"$set": {"credits_reset_at": _long_ago()}})
...
class TestBalanceSurfaces:
    ...
    async def test_spendable_sums_both_buckets(self) -> None:
        user = await make_user("surfaces-sum@example.com", credits_balance=30.0, purchased_credits=70.0)
        assert credits_service.spendable(user) == 100.0

    async def test_spendable_tolerates_a_document_without_the_new_field(self) -> None:
        user = await make_user("surfaces-legacy@example.com", credits_balance=30.0)
